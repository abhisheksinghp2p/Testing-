...
|   5   | Session      |       Data       | Session ID  | Gateway                    | NetBIOS, RPC,                                  | Session management, Authentication(before session establishment) |
...
To ensure the session is still active, systems may send **keep-alive messages periodically**.

Example:
```
Client → Keep-alive signal → Server
Server → Acknowledgement → Client
```

This prevents the session from timing out when no data is being exchanged.

**Important (Real TCP/IP Networks):**

Keep-alive is usually handled by:
...
- WebSocket heartbeat
- Remote desktop heartbeat packets
...
**Provides end-to-end communication, reliability, and flow controhbl**
