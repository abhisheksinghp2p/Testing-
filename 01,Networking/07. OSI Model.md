...
==The Application Layer is the topmost layer of the OSI model that provides network services directly to user applications like web browsers==
...
==The Presentation Layer is responsible for formatting data so both sender and receiver can understand it. It also performs translation, encryption, and compression.==
...
==The Session Layer is responsible for managing sessions by establishing, maintaining, and terminating communication between devices.==
...
==The Transport Layer provides end-to-end communication and ensures reliability, flow control, and error control. It uses protocols like TCP and UDP.==
...
==REPORTS errors and network conditions (does NOT correct them)==
```
+----------------------------------------------------------+
|             ERROR HANDLING & DIAGNOSTICS                  |
+----------------------------------------------------------+
...
```
	...
	Network Feedback:
	    ...
	    "Slow down, network congested" (Source Quench - deprecated)
	    "Redirect - use this route instead"
...
==Responsible for Physical Addressing and Delivers data within the same local area network using MAC Address.==

Provides node-to-node data transfer and handles errors from the Physical Layer.

It makes Raw bits from layer 1 into frames and help detect error in delivery.
...
##### Framing (FCS=Frame Check Sequence)
```
+----------------------------------------------------------+
|                       FRAMING                             |
+----------------------------------------------------------+
