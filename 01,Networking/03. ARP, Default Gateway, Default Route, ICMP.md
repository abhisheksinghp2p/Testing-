...
==A Default Gateway is the IP address of the router that a device uses to send traffic to other networks (outside its local network).==
...
```
Destination is NOT in my network
→ Remote network
```
...
### 4. Host needs MAC address of the gateway

If the MAC is not in the ARP cache, the host sends:
...
Frame is sent to the router.

### 6. Router receives packet

Router checks its **routing table** and forwards the packet toward the destination network.

Router creates a **new Ethernet frame**:
